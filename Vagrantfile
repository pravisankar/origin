...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  vagrant_openshift_config = {
    ...
    "dev_cluster"       => false,
    "insert_key"        => true,
    "num_nodes"         => ENV['OPENSHIFT_NUM_MINIONS'] || 2,
  ...
  if dev_cluster
    ...
    num_node = (vagrant_openshift_config['num_nodes'] || ENV['OPENSHIFT_NUM_MINIONS'] || 2).to_i
    ...
    # OpenShift node
    num_node.times do |n|
      config.vm.define "#{VM_NAME_PREFIX}minion-#{n+1}" do |node|
        node_index = n+1
        node_ip = node_ips[n]
        ...
        node.vm.hostname = "openshift-minion-#{node_index}"
      end
    end
