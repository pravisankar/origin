...
func (master *OsdnMaster) watchNodes() {
	...
	nodeInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			node := obj.(*kapi.Node)
			master.handleAddOrUpdateNode(node, watch.Added)
		},
		UpdateFunc: func(_, cur interface{}) {
			node := cur.(*kapi.Node)
			master.handleAddOrUpdateNode(node, watch.Modified)
		},
		DeleteFunc: func(obj interface{}) {
			obj, err := getDeletedObjFromInformer(obj, Nodes)
			if err != nil {
				log.Error(err)
				return
			}
			node := obj.(*kapi.Node)
			master.handleDeleteNode(node)
		},
	})
}

func (master *OsdnMaster) handleAddOrUpdateNode(node *kapi.Node, eventType watch.EventType) {
...
func (master *OsdnMaster) handleDeleteNode(node *kapi.Node) {
