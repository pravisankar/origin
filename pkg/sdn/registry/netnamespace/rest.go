...
func (rs *REST) List(ctx kapi.Context, options *kapi.ListOptions) (runtime.Object, error) {
	return rs.registry.ListNetNamespaces(ctx, options)
}

// Watch returns NetNamespaces events via a watch.Interface.
// It implements rest.Watcher.
func (rs *REST) Watch(ctx kapi.Context, options *kapi.ListOptions) (watch.Interface, error) {
	return rs.registry.WatchNetNamespaces(ctx, options)
}
...
func (rs *REST) Update(ctx kapi.Context, obj runtime.Object) (runtime.Object, bool, error) {
	...
	if errs := validation.ValidateNetNamespaceUpdate(oldNetns, netns); len(errs) > 0 {
		return nil, false, errors.NewInvalid(kapi.Kind("NetNamespace"), netns.Name, errs)
	}
	...
	if checkGlobalNs && rs.isGlobalNamespace(netns) {
		...
		*netns.NetID = vnid.GlobalVNID
		...
		if err := rs.vnids.Allocate(*netns.NetID); err != nil {
			el := field.ErrorList{field.Invalid(field.NewPath("netID"), netns.NetID, err.Error())}
			return errors.NewInvalid(api.Kind("NetNamespace"), netns.Name, el)
		}
	} else {
