...
func NewRepair(interval time.Duration, registry netnamespace.Registry, vnidRange vnid.VNIDRange, alloc service.RangeRegistry, projectCache *projectcache.ProjectCache) *Repair {
	return &Repair{
		interval:             interval,
		registry:             registry,
		vnidRange:            vnidRange,
...
func (c *Repair) RunOnce() error {
	...
	if c.checkNetNsForProject {
		// Validate every project has a corresponding netnamespace object
		for _, namespace := range c.projectCache.GetNamespaces() {
	...
	for _, obj := range netnsList {
		...
		case vnidallocator.ErrAllocated:
			...
			if netIDCountMap[*netns.NetID] == 1 {
				runtime.HandleError(fmt.Errorf("unexpected vnid %d allocated error for netnamespace %s", *netns.NetID, netns.NetName))
			}
		case vnidallocator.ErrNotInRange:
			// TODO: send event
			// vnid is broken, reallocate
			runtime.HandleError(fmt.Errorf("the vnid %d for netnamespace %s is not within the vnid range %v; please recreate", *netns.NetID, netns.NetName, c.vnidRange))
