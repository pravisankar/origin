...
var (
	...
	ErrorRetryOperation   = errors.New("retry the operation")
)

// Etcd exposes a VNID Allocator that is backed by etcd.
...
// Release attempts to release the provided id locally and then in etcd.
func (e *Etcd) Release(id int) error {
	e.lock.Lock()
	defer e.lock.Unlock()

	if err := e.alloc.Release(id); err != nil {
		return err
	}

	return e.tryUpdate(func() error {
		// Caller must retry the operation to ensure vnid allocation consistency
		return ErrorRetryOperation
	})
}
