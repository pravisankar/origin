...
// RunProjectCache populates project cache, used by scheduler and project/pod admission controllers.
func (c *MasterConfig) RunProjectCache() {
	glog.Infof("Using default project node selector: %s", c.Options.ProjectNodeSelector)
	projectcache.RunProjectCache(c.KubernetesClient, c.Options.ProjectNodeSelector)
}
