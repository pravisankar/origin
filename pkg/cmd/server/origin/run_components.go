...
func (c *MasterConfig) RunNetIDAllocationController() {
	if c.MultitenantNetworkConfig != nil {
		netConfig := NewMultitenantNetworkConfig(c.Options, c.EtcdHelper)
		repair := vnidcontroller.NewRepair(15*time.Minute, netConfig.NetNamespaceRegistry, netConfig.NetIDRange, netConfig.NetIDRegistry)
