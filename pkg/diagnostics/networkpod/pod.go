...
func getGlobalAndNonGlobalPods(pods []kapi.Pod, vmap map[string]uint32) ([]kapi.Pod, []kapi.Pod) {
	...
	for _, pod := range pods {
		if (vmap == nil) || (vmap != nil && vmap[pod.Namespace] == api.GlobalVNID) {
...
func getLocalAndNonLocalPods(kubeClient *kclient.Client) ([]kapi.Pod, []kapi.Pod, error) {
	...
	for _, pod := range pods {
		if pod.Namespace == util.NetworkDiagnosticNamespace || pod.Namespace == util.NetworkDiagnosticGlobalNamespace {
...
func checkDifferentNodePodToPodConnection(localGlobalPods, localNonGlobalPods, nonlocalGlobalPods, nonlocalNonGlobalPods []kapi.Pod, vmap map[string]uint32, r types.DiagnosticResult) {
	...
	if len(localGlobalPods) > 0 && len(nonlocalGlobalPods) > 0 {
		...
		r.Warn("DPodNet1005", nil, fmt.Sprintf("Skipping pod connectivity test for global projects on different nodes. Reason: Couldn't find 2 global pods."))
	}

	// Applicable to multitenant network
	isMultitenant := (vmap != nil)
	if isMultitenant {
		if len(localNonGlobalPods) > 0 && len(nonlocalNonGlobalPods) > 0 {
			...
			r.Warn("DPodNet1006", nil, fmt.Sprintf("Skipping pod connectivity test for non-global projects on different nodes. Reason: Couldn't find 2 non-global pods."))
		}
...
func checkSameNodePodToPodConnection(globalPods, nonGlobalPods []kapi.Pod, vmap map[string]uint32, r types.DiagnosticResult) {
	...
	if len(globalPods) >= 2 {
		...
		r.Warn("DPodNet1008", nil, fmt.Sprintf("Skipping pod connectivity test for global projects on the same node. Reason: Couldn't find 2 global pods."))
	}
	...
	if isMultitenant {
		if len(nonGlobalPods) >= 2 {
			...
			r.Warn("DPodNet1009", nil, fmt.Sprintf("Skipping pod connectivity test for non-global projects on the same node. Reason: Couldn't find 2 non-global pods."))
		}
