...
func (d CheckExternalNetwork) Check() types.DiagnosticResult {
	...
	externalAddress := "www.google.com"
	kexecer := kexec.New()
	if _, err := kexecer.Command("ping", "-c1", "-W1", externalAddress).CombinedOutput(); err != nil {
		r.Error("DExtNet1001", err, fmt.Sprintf("Pinging external address %q failed. Error: %s", externalAddress, err))
	}
	return r
}
