...
func (d CheckServiceNetwork) Check() types.DiagnosticResult {
	...
	if len(localGlobalPods) > 0 {
		...
		r.Warn("DSvcNet1007", nil, fmt.Sprintf("Skipping service connectivity test for global projects. Reason: Couldn't find a global pod."))
	}

	// Applicable to multitenant network
	isMultitenant := (vnidMap != nil)
	if isMultitenant {
		if len(localNonGlobalPods) > 0 {
			checkPodToServiceConnection(localNonGlobalPods[0], services[0], vnidMap, r)
		} else {
			r.Warn("DSvcNet1008", nil, fmt.Sprintf("Skipping service connectivity test for non-global projects. Reason: Couldn't find a non-global pod."))
		}
	}
	return r
}
